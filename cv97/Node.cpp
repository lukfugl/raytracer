...
bool Node::getExposedValue(char *fieldName, int returnType, void *result)
{
	...
	switch (returnType) {
		...
		case fieldTypeSFBool:
			if (buff[0]=='t' || buff[0]=='T') bResult[0]=true;
			else                              bResult[0]=false;
			break;
		case fieldTypeSFFloat:
			fResult[0] = (float) atof(buff);
			break;
		case fieldTypeSFInt32:
			iResult[0] = (int) atoi(buff);
			break;
		case fieldTypeSFVec2f:
			sscanf(buff, "%f%f", &fResult[0], &fResult[1]);
			break;
		case fieldTypeSFVec3f:
			sscanf(buff, "%f%f%f", &fResult[0], &fResult[1], &fResult[2]);
			break;
		case fieldTypeSFString:
			strcpy(sResult, buff);
			break;
		case fieldTypeSFColor:
			...
			break;
		case fieldTypeSFRotation:
			sscanf(buff, "%f%f%f%f", &fResult[0], &fResult[1], &fResult[2], &fResult[3]);
			break;

		case fieldTypeSFTime:
		case fieldTypeSFImage:
